...
export const wars = pgTable(
  ...
  (table) => [
    index("wars_title_embedding_idx").using(
      "ivfflat",
      table.titleEmbedding.asc().nullsLast().op("vector_cosine_ops")
    ),
  ]
);

export const warsTest = pgTable(
  "wars_test",
  {
    // You can use { mode: "bigint" } if numbers are exceeding js number limitations
    id: bigint({ mode: "number" }).primaryKey().notNull(),
  ...
  (table) => [
    index("wars_test_title_embedding_idx").using(
      "ivfflat",
      table.titleEmbedding.asc().nullsLast().op("vector_cosine_ops")
    ),
  ]
